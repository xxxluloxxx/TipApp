...
// Sección 4.2 de debts-ux.md: alta/edición de persona de deuda, 100%
...
const form = reactive({
   ...
   * explícitamente (sección 4.2) — `hasChosenColor` cubre ese tercer estado
...
function onSubmit() {
  ...
  try {
    ...
    // 4.2) — acá solo se decide qué mutación disparar.
    if (props.person) {
      debtPeopleStore.updatePerson(props.person.id, payload)
    } else {
      debtPeopleStore.addPerson(payload)
    }
...
<template>
  <Sheet :open="props.open" @update:open="handleOpenChange">
    <SheetContent side="bottom">
      ...
      <form id="debt-person-form" class="flex flex-col gap-4 px-4" novalidate @submit.prevent="onSubmit">
        <div class="flex flex-col gap-1.5">
          <Label for="nombre-persona-deuda">Nombre</Label>
          <Input
            id="nombre-persona-deuda"
            ref="nameInputRef"
            v-model="form.name"
            placeholder="Ej. Paul"
            maxlength="40"
        ...
        <div class="flex flex-col gap-1.5">
          <Label id="color-persona-deuda-label">Color <span class="font-normal text-muted-foreground">(opcional)</span></Label>
          <div id="color-persona-deuda" role="group" aria-labelledby="color-persona-deuda-label" class="flex flex-wrap gap-3">
            <button
      ...
      <SheetFooter>
        <Button type="submit" form="debt-person-form" class="w-full" :disabled="isSaving">
