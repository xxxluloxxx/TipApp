...
<template>
  <div class="min-h-screen bg-background text-foreground">
    ...
    <main class="mx-auto flex max-w-2xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
      <!-- Estado de carga: mismo layout de Cards apiladas del estado final,
           para que no haya un salto de forma al terminar de cargar. -->
      <template v-if="isInitialLoading">
        <p class="px-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">
          Tus cuentas
        </p>
      ...
      <template v-else>
        ...
        <!-- "Tus cuentas" pasó a ser solo el encabezado + acciones (pedido
             del usuario: cada cuenta en su propia tarjetita) — el listado de
             abajo son Cards independientes, no un `Card` que las contenga a
             todas. -->
        ...
        <!-- Modo normal: cada cuenta en su propia Card, separadas por
             `gap-3` — fila navegable con saldo + chevron + menú ⋮. El tinte
             de color se aplica directo sobre la Card vía `:style` (gana por
             especificidad sobre `bg-card`). -->
